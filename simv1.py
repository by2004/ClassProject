...
def create_config_file(path,dcu,nsc,nrg,ral,wfs,mwg,mwf,sch,lds):
        pathf = path+"/m2sconf.conf"
	conf_file = open(pathf,'w')
...
if __name__ == "__main__":
        ...
        path_in = "/afs/nd.edu/user29/hsumerka/cse60321/res/"
        dev_NumComputeUnits = [1,5,10,15,20,40]
                                                        ...
                                                        for g in max_wf_pcu:
                                                                for h in sched:
                                                                        for j in lds_sizes:
                                                                                ...
                                                                                sch = h
                                                                                lds = j
										benchmarks_x =  [1,3,4,5,6,9,10,12]
										index_f = index_f+1
										path_x = path+"/summary"+str(a)+"_"+str(b)+"_"+str(c)+"_"+str(d)+"_"+str(e)+"_"+str(f)+"_"+str(g)+"_"+str(h)+"_"+str(j)+"_"+str(4)+".txt"
										if index_f < 1000:
											if not os.path.exists(path_x):
												print path_x
																								
												create_config_file(path,dcu,nsc,nrg,ral,wfs,mwg,mwf,sch,lds)
                            	                                                    		benchmarks =  [1,3,4,5,6,9,10,12]
                                        	                                       		work_queue1 = Queue()
                                    	                                           		work_queue2 = Queue()
                                                	                               		for z in benchmarks:
                                        ...
                                        for e in wf_size:
                                                for f in max_wg_pcu: 
                                                        for g in max_wf_pcu:
                                                                for h in sched:
                                                                        for j in lds_sizes:
                                                                                benchmarks = [1,3,4,5,6,8,9,10,11,12]
                                                                                for z in benchmarks:
                                                                                        sim_id = str(a)+"_"+str(b)+"_"+str(c)+"_"+str(d)+"_"+str(e)+"_"+str(f)+"_"+str(g)+"_"+str(h)+"_"+str(j)+"_"+str(z)
